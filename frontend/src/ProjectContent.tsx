...
const ImageBlock: PortableTextTypeComponent = ({
  value,
}: {
  value: ImageFigure;
}) => (
  <figure className="border-[1px] border-[currentColor]">
    <img
      src={getSanityImageUrlFor(value?.image?.asset as SanityImageSource)
        .width(1000)
        .url()}
      alt={value?.alt}
    />
...
const H3Block: PortableTextBlockComponent = ({ children }) => (
  <h3 className="my-4 font-mono">{children}</h3>
);
const LinkBlock = ({
  value,
}: {
  ...
  <a href={value.url} className="block underline">
    {value.text}
  </a>
);
...
const QuoteBlock = ({
  ...
  <figure>
    ...
    <figcaption>
      ...
      <img
        src={getSanityImageUrlFor(value.headshot).width(800).url()}
        alt="hi"
      />
    </figcaption>
  </figure>
